...
function simpleInpaint(
  ...
  const fill = n > 0 ? `rgb(${Math.round(r / n)},${Math.round(g / n)},${Math.round(b / n)})` : "#f5f5f5";
  ctx.save();
  ctx.fillStyle = fill;
...
export default function Home() {
  ...
  const [edit, setEdit] = useState<EditState>({
    text: "", fontSize: 32, fontFamily: "Inter, sans-serif", color: "#111111",
    offsetX: 0, offsetY: 0, rotation: 0, inpaintRadius: 6,
  });
  ...
  const runOcr = async () => {
    ...
    try {
      ...
      for (const w of words) {
        ...
        const bbox: BBox = { x0: w.bbox.x0, y0: w.bbox.y0, x1: w.bbox.x1, y1: w.bbox.y1 };
        ...
        items.push({
          ...
          autoSize: estimateFontSize(bbox), autoColor, autoFont: guessFontFamily(w.text),
        });
      }
      if (items.length === 0 && result.data.lines) {
        for (const line of result.data.lines) {
          if (!line.text.trim()) continue;
          const bbox: BBox = { x0: line.bbox.x0, y0: line.bbox.y0, x1: line.bbox.x1, y1: line.bbox.y1 };
          let autoColor = "#111111";
          if (ctx) autoColor = getDominantColor(ctx, bbox.x0, bbox.y0, bbox.x1, bbox.y1);
          items.push({
            id: generateId(), text: line.text.trim(), confidence: line.confidence || 70, bbox,
            autoSize: estimateFontSize(bbox), autoColor, autoFont: guessFontFamily(line.text),
          });
        }
      }
      setDetected(items);
      if (items.length > 0) {
        selectText(items[0]);
        setStatus(`✅ Found ${items.length} text regions • Click any text on image to edit`);
      } else {
  ...
  const selectText = (item: DetectedText) => {
    ...
    setEdit({
      ...
      inpaintRadius: Math.max(4, Math.round(Math.max(item.bbox.x1 - item.bbox.x0, item.bbox.y1 - item.bbox.y0) * 0.06)),
    });
    setStatus(`Selected: "${item.text}" • size=${item.autoSize}px • ${item.autoColor}`);
  };

  // Click on image → select text under cursor (same font/size/color)
  const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    else setStatus("No text at this position. Click inside a yellow/cyan box.");
  };

  const handleCanvasMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    const canvas = canvasRef.current;
    if (!canvas || detected.length === 0) {
    ...
    for (const d of detected) {
      const { x0, y0, x1, y1 } = d.bbox;
      if (x >= x0 - 4 && x <= x1 + 4 && y >= y0 - 4 && y <= y1 + 4) { over = true; break; }
    }
  ...
  const applyEdit = () => {
    ...
    img.onload = () => {
      ...
      setStatus("✅ Edit applied • Click next text on image or Download");
    };
  ...
  const addNewText = () => {
    ...
    const item: DetectedText = {
      ...
      autoSize: 32, autoColor: "#111111", autoFont: "Inter, sans-serif",
    };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-950 text-slate-100">
      <header className="border-b border-slate-800 bg-slate-900/80 backdrop-blur sticky top-0 z-20">
        <div className="max-w-[1600px] mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-semibold text-lg leading-tight">AI Image Text Editor</h1>
              <p className="text-xs text-slate-400">Click text on image • Auto font match</p>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-wrap justify-end">
            <button onClick={() => fileInputRef.current?.click()} className="btn bg-blue-600 hover:bg-blue-500">
              <Upload className="w-4 h-4" /> Open
            </button>
            <button onClick={runOcr} disabled={isOcrRunning || !imageSrc} className="btn bg-violet-600 hover:bg-violet-500 disabled:opacity-50">
              {isOcrRunning ? <Loader2 className="w-4 h-4 animate-spin" /> : <ScanSearch className="w-4 h-4" />}
              Detect Text
            </button>
            <button onClick={applyEdit} disabled={!selected} className="btn bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50">
              <Check className="w-4 h-4" /> Apply
            </button>
            <button onClick={undo} disabled={history.length === 0} className="btn bg-amber-600 hover:bg-amber-500 disabled:opacity-50">
              <Undo2 className="w-4 h-4" /> Undo
            </button>
            <button onClick={addNewText} disabled={!imageSrc} className="btn bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50">
              <Plus className="w-4 h-4" /> Add Text
            </button>
            <button onClick={download} disabled={!imageSrc} className="btn bg-green-600 hover:bg-green-500 disabled:opacity-50">
      ...
      <div className="flex-1 flex flex-col lg:flex-row max-w-[1600px] w-full mx-auto">
        <aside className="w-full lg:w-[340px] border-r border-slate-800 bg-slate-900/50 p-4 space-y-5 overflow-y-auto max-h-[calc(100vh-64px)]">
          <section>
            ...
            {detected.length === 0 ? (
              <p className="text-sm text-slate-500 mt-2">No text yet. Upload → Detect Text → Click on image</p>
            ) : (
          ...
          <section className={selected ? "" : "opacity-50 pointer-events-none"}>
            <h2 className="section-title"><Sparkles className="w-4 h-4" /> Edit & Auto Match</h2>
            ...
            <label className="label mt-3 flex justify-between"><span>Font Size</span><span className="text-cyan-400">{edit.fontSize}px</span></label>
            <input type="range" min={10} max={200} value={edit.fontSize} onChange={(e) => setEdit({ ...edit, fontSize: Number(e.target.value) })} className="w-full accent-cyan-500" />
            <label className="label mt-3 flex items-center gap-2"><Palette className="w-3.5 h-3.5" /> Color (Auto detected)</label>
            <div className="flex items-center gap-3">
            ...
            <label className="label mt-3 flex justify-between"><span className="flex items-center gap-1"><Move className="w-3.5 h-3.5" /> Offset X</span><span>{edit.offsetX}px</span></label>
            ...
            <label className="label mt-2 flex justify-between"><span>Offset Y</span><span>{edit.offsetY}px</span></label>
            ...
            <label className="label mt-3 flex justify-between"><span className="flex items-center gap-1"><Eraser className="w-3.5 h-3.5" /> Erase Radius</span><span>{edit.inpaintRadius}</span></label>
            <input type="range" min={1} max={30} value={edit.inpaintRadius} onChange={(e) => setEdit({ ...edit, inpaintRadius: Number(e.target.value) })} className="w-full accent-cyan-500" />
          </section>
          ...
          <div className="text-xs text-slate-500 bg-slate-800/40 rounded-lg p-3 leading-relaxed">
            ...
            <ol className="list-decimal ml-4 mt-1 space-y-1">
              <li>Upload image</li>
              <li>Click Detect Text</li>
              <li><b>Click any text on image</b> to select (auto font/size/color)</li>
              <li>Edit → Apply → next text</li>
              <li>Download when done</li>
            </ol>
          </div>
        </aside>

        <main className="flex-1 relative canvas-container flex items-center justify-center p-4 overflow-auto min-h-[50vh]"
          ...
          {!imageSrc ? (
            <div onClick={() => fileInputRef.current?.click()}
              ...
              <p className="text-lg font-medium text-slate-300">Drop image here or click to upload</p>
            ...
            <canvas
              ...
              className="max-w-full max-h-[calc(100vh-120px)] rounded-lg shadow-2xl shadow-black/40 cursor-crosshair"
            />
          )}
        </main>
      </div>
      ...
      <style jsx global>{`
        .btn { @apply inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition text-white; }
